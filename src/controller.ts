...
export class KernelManager {
  ...
  refresh() {
    ...
    for (const [name, kernel] of this.controllers) {
      if (!currentNames.has(name)) {
        ...
        for (const [
          uri,
          mappedKernel,
        ] of this.selectedKernelByNotebook.entries()) {
    ...
    for (const conn of connections) {
      if (this.controllers.has(conn.name)) {
        ...
        const kernel = new SQLNotebookKernel(
          conn,
          this.context,
          this.parameterProvider,
        );
        this.controllers.set(conn.name, kernel);
        const selectionDisposable = kernel.onDidChangeSelectedNotebooks(
          ({ notebook, selected }) => {
  ...
  public getKernelForNotebook(
    notebook: vscode.NotebookDocument | undefined,
  ): SQLNotebookKernel | undefined {
    if (!notebook) {
      return undefined;
    }
    const uri = notebook.uri.toString();
...
export class SQLNotebookKernel {
  ...
  public updateConfiguration(newConfig: ConnData) {
    ...
    if (this.pool) {
      console.log(
        `[SQL Notebook] Recycling connection for:: ${this.config.name}`,
      );
      this.pool.end();
      this.pool = null;
    }
  ...
  public async getSchemaOrLoad(): Promise<TableSchema[]> {
    ...
    const autoFetch =
      vscode.workspace
        .getConfiguration('sqlnotebook')
        .get<boolean>('autoFetchSchema') ?? true;
    if (!autoFetch) {
      return [];
    }
    try {
  ...
  private updateDescription() {
    this._controller.label = this.config.group
      ? `${this.config.group} / ${this.config.name}`
      : this.config.name;
    ...
    if (!this.schemaCache) {
      const autoFetch =
        vscode.workspace
          ...
          .get<boolean>('autoFetchSchema') ?? true;
      if (autoFetch) {
        this.getSchemaOrLoad().then(() =>
          console.log('Schema pre-fetched after execution'),
        );
      }
    }
  }
  private async getPool(): Promise<Pool> {
    ...
    if (!password && this.config.driver !== 'sqlite') {
      try {
        password =
          (await this.context.secrets.get(this.config.passwordKey)) ||
          undefined;
      } catch (e) {}
    }
    let timeoutSeconds =
  ...
  private async doExecution(cell: vscode.NotebookCell, isExplainPlan: boolean = false): Promise<void> {
    ...
    const PARAMS_REGEX =
      /\/\*\s*<SQL_PARAMS>\s*([\s\S]*?)\s*<\/SQL_PARAMS>\s*\*\//;
    rawQuery = rawQuery.replace(PARAMS_REGEX, '').trim();
    const params = this.parameterProvider.getParameters(
    ...
    const safeDelete =
      vscode.workspace.getConfiguration('sqlnotebook').get('safeDelete') ??
      true;
        ...
        if (safeDelete) {
          ...
          if ((isDelete || isUpdate) && !/\bWHERE\b/i.test(strippedBatch)) {
            ...
            vscode.window.showErrorMessage(
              `Safety Alert: ${cmd} without WHERE blocked.`,
            );
            await writeErr(execution, errMsg);
            return;
          }
  ...
  private async appendExecutionResult(
    ...
    const maxRows =
      vscode.workspace
        .getConfiguration('sqlnotebook')
        .get<number>('maxResultRows') ?? 10000;
    const tableMatch =
      query.match(/\bFROM\s+([#\w.\[\]"`]+)/i) ||
      query.match(/\bUPDATE\s+([#\w.\[\]"`]+)/i) ||
      query.match(/\bINSERT\s+INTO\s+([#\w.\[\]"`]+)/i);
    ...
    if (tableName && this.schemaCache) {
      const cleanTableName = tableName
        ...
        .split('.')
        .pop();
      const tableMeta = this.schemaCache.find(
        (t) => t.table.toLowerCase() === cleanTableName?.toLowerCase(),
      );
      if (
        tableMeta &&
...
function resolveParameter(param: StoredParameterValue): {
  value: string;
  raw: boolean;
} {
...
function makeExecutionInfo(date: Date) {
  ...
  const badgeKeywords = {
    danger: config
      .get<string>(
        'badgeKeywordsDanger',
        '🔴, atrasada, failed, fail, error, critical, cancelado, cancelled, rechazado, rejected, timeout',
      )
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
    warning: config
      .get<string>(
        'badgeKeywordsWarning',
        '🟡, urgente, warning, pending, en pausa, paused, en espera, waiting, delayed, demorado',
      )
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
    success: config
      .get<string>(
        'badgeKeywordsSuccess',
        '🟢, a tiempo, success, ready, ok, completed, done, activo, active, terminado, finished, aprobado, approved, entregado',
      )
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
    inactive: config
      .get<string>(
        'badgeKeywordsInactive',
        '⚪, sin fecha, inactive, null, none, cerrado, closed, disabled, desactivado, archived, archivado, n/a, empty',
      )
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
    processing: config
      .get<string>(
        'badgeKeywordsProcessing',
        '🔵, processing, running, en progreso, in progress, en proceso, started, iniciado, cargando, loading',
      )
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
