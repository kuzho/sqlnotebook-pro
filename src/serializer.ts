...
async function cleanupEmbeddedImageSources(embeddedFiles: string[], notebookDir?: string): Promise<void> {
  if (!notebookDir || embeddedFiles.length === 0) {
    return;
  }

  const cleanupEnabled = vscode.workspace
    .getConfiguration('sqlnotebook')
    .get<boolean>('cleanupEmbeddedImageFiles', true);
  ...
  const normalizedNotebookDir = path.resolve(notebookDir);

  await Promise.all(embeddedFiles.map(async (embeddedFile) => {
    const absolutePath = path.resolve(embeddedFile);
    if (path.dirname(absolutePath) !== normalizedNotebookDir) {
      return;
    }
    ...
    try {
      ...
      // Ignore cleanup failures. The notebook already has embedded data.
    }
  }));
}
...
export class SQLSerializer implements vscode.NotebookSerializer {
  ...
  async serializeNotebook(
    ...
    const filesToCleanup = [...embeddedSourceFiles];
    if (filesToCleanup.length > 0) {
      const cleanupDelayMs = 1500;
      setTimeout(() => {
        void cleanupEmbeddedImageSources(filesToCleanup, notebookDir);
      }, cleanupDelayMs);
    }
