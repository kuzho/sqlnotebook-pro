...
export class KeyItem extends vscode.TreeItem {
  constructor(public readonly columnName: string, keyType: string) {
...
export class ForeignKeyItem extends vscode.TreeItem {
  constructor(public readonly fk: import('./driver').ForeignKey) {
    super(`${fk.column} → ${fk.referencedTable}.${fk.referencedColumn}`, vscode.TreeItemCollapsibleState.None);
    this.contextValue = 'foreign-key';
    this.iconPath = new vscode.ThemeIcon('link');
    this.tooltip = `References ${fk.referencedTable}(${fk.referencedColumn})` + (fk.referencedSchema ? ` in schema ${fk.referencedSchema}` : '');
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    if (element instanceof ConnectionListItem) {
      try {
        ...
        schema.forEach(obj => {
          if (obj.schema) {
            if (!schemaGroups.has(obj.schema)) {schemaGroups.set(obj.schema, []);}
        ...
        if (orphans.length > 0) {
          ...
          if (tables.length)
            {items.push(new ObjectGroupItem('Tables', tables, element.config, 'table'));}
          if (views.length)
            {items.push(new ObjectGroupItem('Views', views, element.config, 'view'));}
          if (procs.length)
            {items.push(new ObjectGroupItem('Procedures', procs, element.config, 'procedure'));}
          if (funcs.length)
            {items.push(new ObjectGroupItem('Functions', funcs, element.config, 'function'));}
        }
        if (items.length === 0) {
          return [new vscode.TreeItem("No objects found", vscode.TreeItemCollapsibleState.None)];
        }
        return items;
      } catch (e: any) {
    ...
    if (element instanceof SchemaItem) {
      ...
      if (funcs.length)
        {items.push(new ObjectGroupItem('Functions', funcs, element.config, 'function'));}
      if (items.length === 0) {
        return [new vscode.TreeItem('No objects in schema', vscode.TreeItemCollapsibleState.None)];
      }
      return items;
    }
    if (element instanceof ObjectGroupItem) {
      return element.objects.map(obj => createObjectTreeItem(obj, element.config));
    }
    if (element instanceof TableItem) {
      const children: vscode.TreeItem[] = [];
      if (element.tableSchema.columns && element.tableSchema.columns.length > 0) {
        children.push(new ColumnsGroupItem(element.tableSchema, element.config));
      }
      if (element.tableSchema.primaryKeys && element.tableSchema.primaryKeys.length > 0) {
        children.push(new PrimaryKeysGroupItem(element.tableSchema, element.config));
      }
      if (element.tableSchema.foreignKeys && element.tableSchema.foreignKeys.length > 0) {
        children.push(new ForeignKeysGroupItem(element.tableSchema, element.config));
      }
      return children.length ? children : [new vscode.TreeItem("No details", vscode.TreeItemCollapsibleState.None)];
    }
    ...
    if (element instanceof PrimaryKeysGroupItem) {
      return element.tableSchema.primaryKeys!.map(pk => new KeyItem(pk, 'Primary Key'));
    }
    if (element instanceof ForeignKeysGroupItem) {
      return element.tableSchema.foreignKeys!.map(fk => new ForeignKeyItem(fk));
    }
...
export class SchemaItem extends vscode.TreeItem {
  constructor(public readonly schemaName: string, public readonly objects: TableSchema[], public readonly config: ConnData) {
...
export class ObjectGroupItem extends vscode.TreeItem {
  constructor(label: string, public readonly objects: TableSchema[], public readonly config: ConnData, public readonly type: string) {
    super(label, vscode.TreeItemCollapsibleState.Collapsed);
    this.contextValue = `object-group-${type}`;
    this.iconPath = new vscode.ThemeIcon(type === 'table' ? 'table' : type === 'view' ? 'eye' : type === 'procedure' ? 'symbol-method' : 'symbol-function');
  }
}
...
function createObjectTreeItem(obj: TableSchema, config: ConnData): vscode.TreeItem {
  switch (obj.type) {
    case 'table': return new TableItem(obj, config);
    case 'view': return new ViewItem(obj, config);
    case 'procedure': return new ProcedureItem(obj, config);
    case 'function': return new FunctionItem(obj, config);
    default: return new TableItem(obj, config);
  }
}
