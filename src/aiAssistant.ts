    ...
    .map((s) => {
      ...
      if (s.foreignKeys && s.foreignKeys.length > 0) {
        s.foreignKeys.forEach((fk) => {
          cols.push(
            `  FOREIGN KEY (${fk.column}) REFERENCES ${fk.referencedSchema ? fk.referencedSchema + '.' : ''}${fk.referencedTable}(${fk.referencedColumn})`,
          );
        });
      }
  ...
  const handler: vscode.ChatRequestHandler = async (
      ...
      if (activeCell && activeCell.kind === vscode.NotebookCellKind.Code) {
        ...
        if (
          ...
          activeTextEditor.document.uri.toString() ===
            activeCell.document.uri.toString()
        ) {
          if (!activeTextEditor.selection.isEmpty) {
            activeContextText = activeTextEditor.document.getText(
    ...
    if (activeKernel && activeKernel.getDriver()) {
      try {
        ...
        let historyText = chatContext.history
          .map((turn) => {
            if ('prompt' in turn) {
              return turn.prompt;
            }
        ...
        if (relevantSchemas.length > 0) {
          relevantSchemas.forEach((s) => {
            s.foreignKeys?.forEach((fk) => {
              const related = schema.find(
                (t) => t.table === fk.referencedTable,
              );
              if (related) {
                schemasToSendSet.add(related);
              }
            });
          });
          schema.forEach((s) => {
            if (
              s.foreignKeys?.some((fk) =>
                relevantSchemas.some((rt) => rt.table === fk.referencedTable),
              )
            ) {
        ...
        if (schemasToSend.length > 0 || schema.length <= 8) {
          const finalSchemas =
            schemasToSend.length > 0 ? schemasToSend : schema;
          schemaPrompt =
            'Relevant tables detailed schema (including related tables):\n' +
            formatSchemaForPrompt(finalSchemas);
          const otherTables = schema.filter((s) => !finalSchemas.includes(s));
          if (otherTables.length > 0) {
            schemaPrompt +=
              '\n\nOther available tables (names only): ' +
              otherTables.map((t) => t.table).join(', ');
          }
        } else {
          schemaPrompt =
            'Available tables (names only): ' +
    ...
    response.progress(`Querying ${model.name}...`);
    let fullResponse = '';
    try {
    ...
    const isSafe = statementInfos.every((info) =>
      safeTypes.includes(info.type),
    );
    if (execMode === 'strict' && !isSafe) {
      response.markdown(
        `❌ **Blocked by SQL Notebook Pro (Strict Mode):** The query attempts to modify data. Only safe queries (SELECT, SHOW, DESCRIBE, etc.) are allowed.\n\n\`\`\`sql\n${generatedSql}\n\`\`\``,
      );
      return;
    }
    try {
      ...
      edit.set(activeNotebook.uri, [
        vscode.NotebookEdit.insertCells(index, [cellData]),
      ]);
      await vscode.workspace.applyEdit(edit);
      response.markdown(
        `✅ **Code successfully injected!** You have a new cell in your Notebook.\n\n\`\`\`sql\n${generatedSql}\n\`\`\``,
      );
      const editor = vscode.window.activeNotebookEditor;
      if (
        editor &&
        editor.notebook.uri.toString() === activeNotebook.uri.toString()
      ) {
      ...
      response.markdown(
        `❌ Failed to create cell in your notebook: ${e.message}\n\n\`\`\`sql\n${generatedSql}\n\`\`\``,
      );
    }
  };
  context.subscriptions.push(
    vscode.chat.createChatParticipant('sqlnotebook.aiAssistant', handler),
  );
}
