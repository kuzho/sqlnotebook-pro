...
function getCachedSlicerValues(reportName: string): Record<string, string> {
  return cachedSlicerValues[reportName] ? { ...cachedSlicerValues[reportName] } : {};
}

function persistSlicerValues(reportName: string, slicerValues: Record<string, string> = {}) {
  cachedSlicerValues[reportName] = { ...(cachedSlicerValues[reportName] || {}), ...slicerValues };
}
  ...
  panel.webview.onDidReceiveMessage(async (message) => {
    if (message.type === 'save') {
      ...
      try {
        ...
        if (!reportName) {
          vscode.window.showWarningMessage('Cannot update datasets: report name not available');
          return;
        }
        ...
        vscode.window.showErrorMessage('Failed to update datasets: ' + (err?.message || String(err)));
      }
    }
  });
  console.log('openReportBuilder: panel html assigned for report', existingReport?.name || '(new)', new Date().toISOString());
}
...
async function handleOpenReport(report: ReportData, context: vscode.ExtensionContext, kernelManager: KernelManager) {
  ...
  const runQuery = async (slicerValues: Record<string, string> = {}) => {
    ...
    try {
      const results = await Promise.all(activeReport.datasets.map(async (ds) => {
        ...
        try {
          ...
          Object.entries(slicerValues).forEach(([name, val]) => {
            ...
            finalQuery = finalQuery.replace(new RegExp(escapeRegExp(safeName), 'g'), `'${escapedVal}'`);
          });
