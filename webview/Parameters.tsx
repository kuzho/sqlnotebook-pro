...
const unformatSqlValue = (sqlValue: string): string => {
  ...
  const items = processableString.split(',').map((item) => {
...
const Parameters: React.FC = () => {
  ...
  const toParameter = React.useCallback(
    (
      key: string,
      rawParam: IncomingStoredParameter,
      index: number,
    ): Parameter => {
      ...
      if (
        rawParam?.type === 'checkbox' ||
        rawParam?.type === 'select' ||
        rawParam?.type === 'date'
      ) {
      ...
      const options = Array.isArray(rawParam?.options)
        ? rawParam.options.map((v) => String(v)).join(',')
        : '';
      const checked =
        typeof rawParam?.checked === 'boolean'
          ? rawParam.checked
          : value === checkedValue;
  ...
  const outgoingParameters = React.useMemo(
    () => buildOutgoingParameters(parameters),
    [buildOutgoingParameters, parameters],
  );
  ...
  React.useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message.type === 'set_parameters') {
        ...
        const newParamsArray = Object.entries(newParamsObj || {}).map(
          ([key, val], idx) =>
            toParameter(key, val as IncomingStoredParameter, idx),
        );
      ...
      if (message.type === 'update_state') {
        const { isDirty: nextIsDirty, hasActiveFile: nextHasActiveFile } =
          message.payload;
  ...
  const handleParameterChange = (
    ...
    if (field === 'name') {
      ...
      if (candidate) {
        const duplicated = parameters.some(
          (p) =>
            p.id !== id &&
  ...
  const handleTypeChange = (
    id: number,
    nextType: 'text' | 'checkbox' | 'select' | 'date',
  ) => {
    setIsDirty(true);
    setParameters(
      parameters.map((p) => {
  ...
  const handleListScroll = React.useCallback(
    (e: React.UIEvent<HTMLDivElement>) => {
      ...
      if (el.scrollTop + el.clientHeight >= el.scrollHeight - 80) {
        setVisibleCount((prev) =>
          Math.min(parameters.length, prev + LIST_BATCH_SIZE),
        );
      }
    },
    [parameters.length],
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: '11px', opacity: 0.75, marginTop: '4px' }}>
          Showing {Math.min(visibleCount, parameters.length)} of{' '}
                    ...
                    onInput={(e: any) => {
                      ...
                      setParameters((prev) =>
                        prev.map((existing) => {
                          ...
                          const nextValue = options.includes(existing.value)
                            ? existing.value
                            : options[0] || '';
